...
// دالة لتمديد البطاقة
function expandDevCard(devId) {
    // جلب النصف الذي تم الضغط عليه
    const targetHalf = document.getElementById('half-' + devId);
    
    // إذا كان أصلاً ممدداً، لا تفعل شيء
    if(targetHalf.classList.contains('active')) return;

    // إضافة كلاس active ليتمدد (clip-path)
    targetHalf.classList.add('active');
    
    // إخفاء زر الإغلاق الرئيسي للمودال لكي لا يتداخل
    document.querySelector('.global-close').style.display = 'none';
}

// دالة للعودة لوضع المثلثات
function collapseDevCard(event, devId) {
    // منع تفعيل دالة expandDevCard عند الضغط على زر الرجوع
    event.stopPropagation(); 
    
    // إزالة كلاس التمدد ليرجع لشكله المثلث
    const targetHalf = document.getElementById('half-' + devId);
    targetHalf.classList.remove('active');

    // إرجاع زر الإغلاق الرئيسي للمودال بعد انتهاء الأنيميشن
    setTimeout(() => {
        document.querySelector('.global-close').style.display = 'block';
    }, 500); // 500ms هو وقت الأنيميشن
}